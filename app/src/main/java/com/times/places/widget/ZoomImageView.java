package com.times.places.widget;
...
public class ZoomImageView extends NetworkImageView {
	...
	/**
	 * Display an image from remote location
	 */
	public void setNetworkImage(int defaultImageRes, String url,
	...
	public void setScaleType(ScaleType type) {
		...
		if (type == ScaleType.MATRIX) {
			...
			if (onDrawReady) {
				 ...
				 * in program and the TouchImageView should be updated with the
				 * new scaleType.
				 */
				setZoom(this);
			}
	...
	public void setZoom(float scale, float focusX, float focusY,
		 ...
		 * point,image and view sizes have not yet been calculated in onMeasure.
		 * Thus, we should delay calling setZoom until the view has been
		 * measured.
		 */
		if (!onDrawReady) {
	...
	private float
			getFixDragTrans(float delta, float viewSize, float contentSize) {
		if (contentSize <= viewSize) {
			return 0;
		}
		return delta;
	...
	private void fitImageToView() {
		...
		if (!isZoomed() && !imageRenderedAtLeastOnce) {
			 ...
			 * viewHeight to NaN in translateMatrixAfterRotate. To avoid this,
			 * call savePreviousImageValues to set them equal to the current
			 * values.
			 */
			...
			// Re-scale Matrix after rotation
	...
	private void translateMatrixAfterRotate(int axis, float trans,
		...
		if (imageSize < viewSize) {
			 ...
			 * view. Determine its distance from the left/top side of the view
			 * as a fraction of the entire image's width/height. Use that
	...
	private class GestureListener
		...
		public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
			...
			if (fling != null) {
				 ...
				 * so that two flings are not run simultaneously.
				 */
				fling.cancelFling();
			}
	...
	private class Fling implements Runnable {
		...
		public void run() {
			 ...
			 * flung by user. Listener runnable updated with each frame of fling
			 * animation.
			 */
			if (touchImageViewListener != null) {
				touchImageViewListener.onMove();
